...
resource "aws_security_group" "project1webservers"{
  tags = {
    Name = "${var.ENVIRONMENT}-project1-webservers"
  }
...
resource "aws_launch_configuration" "launch_config_webserver" {
  ...
  security_groups = [aws_security_group.project1webservers.id]
  key_name = aws_key_pair.project1key.key_name
...
resource "aws_autoscaling_group" "project1webserver" {
  name                      = "project1WebServers"
  max_size                  = 2
  min_size                  = 1
...
resource "aws_lb" "project1-load-balancer" {
  name               = "${var.ENVIRONMENT}-project1-lb"
  internal           = false
...
resource "aws_lb_listener" "webserver_listner" {
  load_balancer_arn = aws_lb.project1-load-balancer.arn
  port              = "80"
  protocol          = "HTTP"
...
output "load_balancer_output" {
  value = aws_lb.project1-load-balancer.dns_name
}